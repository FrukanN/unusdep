...
def createApp (connect):
    appname = "unus" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
if __name__ == "__main__":
    ...
    # Kendi Reponu Yazamadın Mı Siktir lan Auto Yok Sana Benim Lann Bu UnusDev #
    ...
    if os.path.isdir("./unususerbot/"):
        rm_r("./unususerbot/")
    repo = Repo.clone_from(slicedString,"./unususerbot/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @UnusUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/FrukanN/UnusUserBot"
    config['WARN_LIMIT'] = "3"
